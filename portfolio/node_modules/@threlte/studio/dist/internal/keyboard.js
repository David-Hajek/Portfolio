 ...
 * ```
 */
export const flattenKeyCombo = (keyCombo) => {
...
export const formatKeyCombo = (keyCombo) => {
    return keyCombo.split(keyDelimiter).join('+');
};
export const createKeyboardControls = (runAction) => {
    const enabled = currentWritable(true);
    const downKeyMap = currentWritable(new Map());
    const upKeyMap = currentWritable(new Map());
    const addKeys = (scope, keyMapItems) => {
        const updateKeyMap = (keyMap, keyCombos) => {
            keyMap.update((k) => {
                for (const [keyCombo, actionId] of keyCombos.entries()) {
                    if (k.has(keyCombo)) {
                        ...
                        console.warn(`"${formattedKeyCombo}" is already used by action "${action?.scope}:${action?.actionId}", skipping …"`);
                    }
                    else {
                        k.set(keyCombo, {
                            scope,
        ...
        const upKeyCombos = new Map();
        const downKeyCombos = new Map();
    ...
    const onKeyDown = (e) => {
        ...
        checkActiveElement: if (!keys.includes('meta') && !keys.includes('ctrl')) {
            ...
            if (!activeElement)
                break checkActiveElement;
            const tagName = activeElement.tagName.toUpperCase();
            if (activeElement && ignoreInputOnActiveElementTagNames.includes(tagName))
                return;
        }
        const flattenedKeyCombo = flattenKeyCombo(keys);
