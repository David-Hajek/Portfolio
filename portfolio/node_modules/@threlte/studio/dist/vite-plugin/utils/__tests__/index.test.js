...
describe('sync utilities', () => {
    ...
    test('remove attribute', () => {
        ...
        const meshNode = findNodeByIndex(input, 0);
        if (!meshNode)
            throw new Error('Mesh node not found');
    ...
    test('update attributes', () => {
        ...
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'mustache1')).toStrictEqual(v1);
        const v2 = 'string';
        upsertAttribute(input, meshNode, 'mustache2', v2, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'mustache2')).toStrictEqual(v2);
        const v3 = ['mustache value'];
        upsertAttribute(input, meshNode, 'mustache3', v3, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'mustache3')).toStrictEqual(v3);
        const v4 = true;
        upsertAttribute(input, meshNode, 'bool1', v4, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'bool1')).toStrictEqual(v4);
        const v5 = false;
        upsertAttribute(input, meshNode, 'bool2', v5, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'bool2')).toStrictEqual(v5);
        const v6 = 'string in bool prop';
        upsertAttribute(input, meshNode, 'bool3', v6, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'bool3')).toStrictEqual(v6);
        const v7 = ['mustache value in bool prop'];
        upsertAttribute(input, meshNode, 'bool4', v7, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'bool4')).toStrictEqual(v7);
        const v8 = true;
        upsertAttribute(input, meshNode, 'string1', v8, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'string1')).toStrictEqual(v8);
        const v9 = 'string2';
        upsertAttribute(input, meshNode, 'string2', v9, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'string2')).toStrictEqual(v9);
        const v10 = ['mustache value in string prop'];
        upsertAttribute(input, meshNode, 'string3', v10, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'string3')).toStrictEqual(v10);
        const v11 = true;
        upsertAttribute(input, meshNode, 'shorthand1', v11, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'shorthand1')).toStrictEqual(v11);
        const v12 = 'string in shorthand prop';
        upsertAttribute(input, meshNode, 'shorthand2', v12, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'shorthand2')).toStrictEqual(v12);
        const v13 = ['mustache value in shorthand prop'];
        upsertAttribute(input, meshNode, 'shorthand3', v13, 'last');
        expect(readAttribute(recreateMagicString(input), findNodeByIndex(recreateMagicString(input), 0), 'shorthand3')).toStrictEqual(v13);
