...
<script lang="ts">import { resolvePropertyPath, useTask } from '@threlte/core';
...
let { objects, key, label, autoUpdate, ref = $bindable(), transform, ...rest } = $props();
...
const { start, stop } = useTask(async () => {
    if (!ref)
        return;
