<script lang="ts">import { Element, Pane, Separator } from 'svelte-tweakpane-ui';
...
import { inspectorScope } from './types';
...
const title = $derived.by(() => {
    if (objectSelection.selectedObjects.length === 0)
        return 'Inspector';
...
$effect(() => {
    if (!pane)
        return;
    const contentEl = pane.element.querySelector('.tp-rotv_c');
    if (!contentEl)
        return;
