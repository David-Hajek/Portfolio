<script lang="ts">import { injectPlugin, useThrelte } from '@threlte/core';
...
const tooltip = $derived.by(() => {
    if (!vitePluginEnabled)
        return 'Vite plugin not found';
    if (!extension.state.enabled)
        return 'Sync disabled';
    if (extension.state.mode === 'manual') {
        if (!extension.state.queue.syncQueue.length)
            return 'Up-to-date';
