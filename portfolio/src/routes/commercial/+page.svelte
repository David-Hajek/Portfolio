	...
	const projects: Project[] = [
		{
			...
			solution:
				'I created a reusable node setup in Houdini, which allowed me to quickly iterate between crashes and made the entire process of creating it a breeze',
			outcome:
				'The finished RBD and VDB were then placed into Blender, where Krystof was able to composit it and create the finished render',
		...
		},
		{
			...
			problem:
				'We needed to create a growth simulation that could be easily controled within blender, as we needed full artistic control as to where and how much vegetation will grow.',
			solution:
		...
		},
		{
			...
			problem:
				'I had the task to create a detailed an high definition model of tweezers that needed to hold a diamond, I knew this model needed to be high poly, as it was the second most important thing in the shot.',
			solution:
		...
		},
		{
			...
			problem:
				'I was tasked with the creation of the environment for a scene, we needed to create a realistic environment, that would complement the jewelery in the middle of the scene.',
			solution:
				"I first started with the modeling of the environment, I used a combination of sculpting, importing heightmaps, and texturing based on its's Z values, to create a realistic arctic environment.",
			outcome:
				'The resulting environment was a perfect base for the scene, it added a sense of realism and dept, while keeping the focus on the jewelery, making the entire scene otheworldly and grounded.',
			videoUrl:
				'https://s3.amazonaws.com/webflow-prod-assets/5d626c045bf4d84a1c256e90/6952875461ca8c9f99315d92_cgi_antartcica-web2.mp4',
			images: [{ url: '/images/commercial/antarctic/antarctica-bts.gif', alt: 'Antarctica BTS' }],
			credits: {
				company: {
					name: 'Krystof Jezek',
					logo: '/images/krystof-logo.png'
		...
		},
		{
			...
			description:
				'A series of background renders that were used for a trailer for a newly released novel',
			problem:
				'I was tasked with the creation of Background renders for a novel trailer, the client needed to maintain a degree of stylization, as there would always be a character occupying the image.',
			solution:
				'I needed to create compositions that would allow for the client to place the characters in the foreground, without the worry of the background taking too much attention away from the voice acted characters. ',
			outcome:
				'The renders created, set the mood of the trailer perfectly, while maintaing the mix of realism and stylization.',
		...
		},
		{
			...
			problem:
				'I was tasked with the creation of a Logo, and helping with particular 3D assets for the video series Krajáč. ',
			solution:
				'As Krajáč is a hockey themed series, I needed to nail the hockey aesthethic, I wanted the logo to scream Hockey. After discussing it further, we picked out a font and I started to create the logo while staying true to the Hockey aesthethic. I also created a transition for the series, where a hockey puck covers the screen and flies out.',
			outcome:
				'The logo was very well received, aswell as the transition, as both fit very well into our own Hockey-Krajáč aesthethic we have created.',
...
<div class="container" in:fade={{ duration: 300, delay: 300 }}>
	<header class="page-header">
		...
		<p class="subtext">
			A comprehensive collection of my favorite projects I had to pleasure to work on.
		</p>
	</header>
							...
							<div class="credits-section">
								...
								<div class="company-info">
									{#if project.credits.company.logo}
										<img
											src={`${base}${project.credits.company.logo}`}
											alt={project.credits.company.name}
											class="company-logo"
										/>
									{/if}
